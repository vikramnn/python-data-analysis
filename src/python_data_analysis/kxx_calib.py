...
def plotCalib(df_calib, thermometers):
    ...
    for ax in axes:
        ax.set_prop_cycle(color=[cmap(1.0 * k / num_colors)
...
def chebyCalibFields(df_avg, cheby_deg, fields, thermometers):
    ...
    for therm in thermometers:
        ...
        for i, field in enumerate(fields):
            ...
            z = (x * (domain_R[1] - domain_R[0]) +
                 domain_R[0] + domain_R[1]) / 2
...
def plotCheby(Rmax, Rmin, coeffs):
    ...
    z = (x * (domain_logR[1] - domain_logR[0]) +
         domain_logR[0] + domain_logR[1]) / 2
...
def thermometerMR(df_avg, thermometers, **kwargs):
    ...
    gb_mr = df_mr.set_index(
        ["Temp_round (K)", "Field (T)"]).groupby("Temp_round (K)")
...
def extractTemperatureSweep(df, group_titles, field, lxx, lxy, w, t, Tp1='T_H',
                            Tm1='T_C1', Tp2='T_C1', Tm2='T_C2', antisym=True):
    if antisym:
        df_extr = df[df['Field (T)'] != 0].set_index(group_titles).groupby(
            group_titles).filter(lambda x: len(x) > 150).groupby(group_titles).mean()

        df_extr['dT_xx'] = df_extr[Tp1] - df_extr[Tm1]
        df_extr['dT_xy'] = df_extr[Tp2] - df_extr[Tm2]

        dT_xy_asym_p = (df_extr.xs(field)[
                        'dT_xy'].values - df_extr.xs(-field)['dT_xy'].values)/2
        dT_xy_asym_m = -dT_xy_asym_p
        df_extr['dT_xy_asym'] = np.append(dT_xy_asym_m, dT_xy_asym_p)

        dT_xx_sym = (df_extr.xs(field)[
                     'dT_xx'].values + df_extr.xs(field)['dT_xx'].values)/2
        ...
        df_extr['kxx_sym'] = (df_extr['Qdot'] * lxx) / \
            (w * t * df_extr['dT_xx_sym'])
        df_extr['kxy'] = -((df_extr['dT_xy_asym'] * w * t) /
                           (df_extr['Qdot'] * lxy)) * df_extr['kxx_sym']**2

    else:
        df_extr = df.set_index(group_titles).groupby(group_titles).filter(
            lambda x: len(x) > 150).groupby(group_titles).mean()
        df_extr['dT_xx'] = df_extr[Tp1] - df_extr[Tm1]
...
def extractFieldSweep(df, group_titles, thermometers, domain_interp_dict,
                      cheby_interp_dict, lxx, lxy, w, t, Tp1='T_H',
                      Tm1='T_C1', Tp2='T_C1', Tm2='T_C2', antisym=True):
    if antisym:
        ...
        # use this if you want to see all up and down sweeps on separate plots
        group_titles = ['Temperature (K)', 'sweep_dir', 'Field (T)']

        for df_extr in [df_extr_pos, df_extr_neg]:
            for therm in thermometers:
                T_string = 'T_' + therm.split('_')[1]
                df_extr[T_string] = df_extr.groupby(['Temperature (K)', 'Field (T)'])[therm].transform(lambda x: extractTempSpline(
                    x.values, x.name[-1], domain_interp_dict[therm][0], domain_interp_dict[therm][1], cheby_interp_dict[therm]))

            df_extr['dT_xx'] = df_extr[Tp1] - df_extr[Tm1]
            df_extr['dT_xy'] = df_extr[Tp2] - df_extr[Tm2]

            df_extr['T_avg (K)'] = (df_extr['T_H'] + df_extr['T_C1'])/2
            df_extr['dT/T'] = df_extr['dT_xx'] / \
                ((df_extr['T_H']+df_extr['T_C1'])/2)

        df_extr_neg = df_extr_neg.sort_index(
            level=['Temperature (K)', 'Field (T)'], ascending=[True, False])
        df_extr_pos = df_extr_pos.reset_index()
        df_extr_neg = df_extr_neg.reset_index()

        df_extr = pd.concat([df_extr_neg[df_extr_neg['sweep_dir'] == 'down'],
                            df_extr_pos[df_extr_pos['sweep_dir'] == 'down']])

        dT_xy_asym_p = (df_extr[df_extr['Field (T)'] > 0]['dT_xy'].values -
                        df_extr[df_extr['Field (T)'] < 0]['dT_xy'].values)/2
        dT_xy_asym_m = -dT_xy_asym_p
        df_extr['dT_xy_asym'] = np.append(dT_xy_asym_m, dT_xy_asym_p)

        dT_xx_sym = (df_extr[df_extr['Field (T)'] > 0]['dT_xy'].values +
                     df_extr[df_extr['Field (T)'] < 0]['dT_xy'].values)/2
        df_extr['dT_xx_sym'] = np.append(dT_xx_sym, dT_xx_sym)

        df_extr['kxx'] = (df_extr['Qdot'] * lxx)/(w * t * df_extr['dT_xx_sym'])
        df_extr['kxy'] = -((df_extr['dT_xy_asym'] * w * t) /
                           (df_extr['Qdot'] * lxy)) * df_extr['kxx']**2
        df_extr['dk/k0'] = df_extr.groupby(group_titles[:-1]
                                           )['kxx'].transform(lambda x: (x-x.min())/x.min())
        df_extr['dk/dH'] = df_extr.groupby(group_titles[:-1])['kxx'].transform(
            lambda x: savgol_filter(x, 7, 3, deriv=1))
    else:
